...
const generateSocialIcons = (socials: Socials, displayLayout: string, badgeStyle: string = 'for-the-badge', gridColumns?: number) => {
  ...
  // Note: GitHub stats are handled separately by the generateGitHubStats function using the GitHub stats SVG API
  // We don't generate a GitHub badge here to avoid duplication
...
// Map theme values to valid GitHub stats API themes
const mapToValidGitHubTheme = (theme: string | undefined): string => {
  ...
  // Direct mapping for valid themes
  if (theme && validThemes.includes(theme)) {
    return theme;
  }
  
  // Map common theme variations to valid themes
  ...
  if (theme && themeMap[theme.toLowerCase()]) {
...
const generateGitHubStats = (stats: SocialStats, config: SocialStatsWidgetConfig) => {
  if (!stats.github || !config.socials.github) return '';
  
  let result = '';
  
  // Generate SVG stats card using github-stats-svg API
...
const generateMarkdown = (config: SocialStatsWidgetConfig, stats: SocialStats) => {
  ...
  // Add GitHub stats using SVG API if GitHub username is configured
...
const SocialStatsWidget: React.FC<SocialStatsWidgetProps> & MarkdownExportable = ({
  ...
  // Handle hydration by ensuring component only renders dynamic content after mounting
  useEffect(() => {
    setMounted(true);
  }, []);  // Fetch GitHub stats when username changes (only after component is mounted)
  useEffect(() => {
    if (!mounted) return;
  ...
  }, [mounted, config.socials?.github]); // Add mounted as dependency  // Update SVG URL when config changes (only after component is mounted)
  useEffect(() => {
    if (!mounted) return;
  ...
  }, [mounted, config.socials?.github, config.theme, config.compactMode, config.hideBorder, config.hideTitle, config.customTitle]);// Generate markdown for the widget and notify parent (only after mounted)
  useEffect(() => {
    if (!mounted) return;
  ...
  }, [mounted, config, stats, onMarkdownGenerated]);
  ...
  const renderGitHubStats = () => {
    if (!stats.github || !config.socials.github) return null;
    
    const displayTitle = config.hideTitle ? null : (config.customTitle || `GitHub Stats`);
    
    // Always render the SVG image for GitHub stats
    return (
        ...
        <div className="flex justify-center">
          <div className="relative">
            {svgPreviewUrl ? (
              <Image
                ...
                onError={(e) => {
                  ...
                  if (retryCount < 3) {
                    ...
                    setError('SVG generation failed after multiple attempts');
                  }
                }}
                onLoad={() => {
                  setSvgLoading(false);
              ...
              <div className="w-96 h-48 bg-gray-100 dark:bg-gray-700 rounded-md flex items-center justify-center">
      ...
      {!mounted ? (
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <button className="px-2.5 py-1.5 text-xs font-medium rounded bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300">
              Preview
            </button>
            <button className="px-2.5 py-1.5 text-xs font-medium rounded bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
              Markdown
            </button>
            <button className="px-2.5 py-1.5 text-xs font-medium rounded bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 flex items-center gap-1">
